...
const ApplicationEditor = ({ application, onClose, onSave, isSaving }) => {
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 2000, display: 'flex', justifyContent: 'center', padding: '20px', overflowY: 'auto' }}>
            <div className="glass-card slide-up" style={{ width: '100%', maxWidth: '600px', height: 'fit-content', position: 'relative', border: '1px solid var(--primary-light)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '1rem' }}>
                    <h3 style={{ margin: 0 }}><i className="fa-solid fa-pen-nib" style={{ color: 'var(--primary-light)' }}></i> Edit Onboarding Draft</h3>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <div className="form-group">
                            ...
                            <input type="text" name="industry" className="input-modern" value={formData.industry} onChange={handleChange} placeholder="e.g. Cafe" required />
                        </div>
                        <div className="form-group">
                            <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'block' }}>Location / Area</label>
                            <input type="text" name="location" className="input-modern" value={formData.location} onChange={handleChange} placeholder="e.g. Bangsar" required />
                        </div>
                    </div>

                    <div className="form-group">
                        <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'block' }}>Address</label>
                        <textarea name="address" className="input-modern" value={formData.address} onChange={handleChange} rows="2" required />
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <div className="form-group">
                            <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'block' }}>Contact Phone</label>
                            <input type="text" name="phone" className="input-modern" value={formData.phone} onChange={handleChange} required />
                        </div>
                        <div className="form-group">
                    ...
                    <div className="form-group">
                        ...
                        <textarea name="purpose" className="input-modern" value={formData.purpose} onChange={handleChange} placeholder="What is the core purpose of this business?" rows="2" />
                    </div>

                    <div className="form-group">
                        <label style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', display: 'block' }}>The Founder's Story</label>
                        <textarea name="story" className="input-modern" value={formData.story} onChange={handleChange} placeholder="Tell us about the journey and convictions..." rows="4" />
                    </div>
